...
public class KShellDecomposition extends NetworkAnalysisAlgorithm {
    
    public KShellDecomposition(){
        algoDescription = "Identifies particular subsets in the network using a recursive pruning strategy";   
    ...
    public HashMap<String,Integer> apply(DefaultGraph graph, AppVariables appVariables){
        ...
        for( int iDeg = 1; iDeg < deg.length; iDeg++){
            nodesCore.put(nodesIndex.get(iDeg),deg[iDeg]);
        }
        long endNanoTime = System.nanoTime();
        long elapsedNanoTime = endNanoTime - startNanoTime;
        double elaspedSecondTime = (double)elapsedNanoTime/(double)1000000000;
        appVariables.addLogEntry("[network analysis] computed k-shell decomposition, "+graph.getNodeCount()+" nodes and "+graph.getEdgeCount()+" edges, in "+formatter.format(elaspedSecondTime)+"s");
        return nodesCore;
    }
    ...
    public String getReference() {
        return "<li><b>K-cores Decomposition:</b> V. Batagelj and M. Zaversnik. An O(m) algorithm for cores decomposition of networks, <i>In University of Ljubljana pre-print series Vol. 40, 799</i>, 2003.</li>";
    }
}
